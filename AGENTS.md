...
Compose files use 2-space YAML indentation. Keep service directories lowercase (`uptimekuma/`), env vars uppercase (`TRAEFIK_HOSTNAME`), and container names prefixed with `stacksmith_` (for example `stacksmith_portainer`). Prefer env-driven configuration for deployment-specific values users actually change: hostnames, credentials, storage paths, resource limits, and feature toggles. New routed services should join the external `stacksmith` network and use Traefik labels patterned after existing services, typically with the `websecure-tailscale` entrypoint and `secure-headers@docker` middleware. For Traefik-only services, do not expose host `ports:` mappings or `*_PORT`/bind-IP variables just to reach the service; hard-code the internal container port in the Traefik service label (for example `loadbalancer.server.port=8000`) unless direct non-Traefik access is an explicit requirement. Default `TZ` in `.env.example` files should remain `Europe/Zurich`.

## Image Tag Policy
Prefer upstream `latest` for application images when available, reducing routine version-bump maintenance. This is a deliberate tradeoff in favour of rolling updates, not permission to deploy automatically. Document justified exceptions (for example database major-version compatibility or specialized builds) in the service README rather than silently pinning applications. Before updating stateful services, preserve a recoverable pre-upgrade backup and record the running version and image digest; moving a tag back does not reverse database migrations.
