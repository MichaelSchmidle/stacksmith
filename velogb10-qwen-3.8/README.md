...
Keep `VELOGB10_DEFAULT_MAX_TOKENS` below `VELOGB10_MAX_SEQ_LEN`. In v0.5.0, a client that omits `max_tokens` inherits the server default; setting it equal to the context length leaves no room for speculative-decoding state and produces an empty `context_length_exceeded` response. Stacksmith's default-budget check requires at least one equally sized prompt budget plus Velo's 16-token speculative-decoding margin. The 8,192-token default therefore leaves safe headroom for Open WebUI and Hermes requests.

Client context metadata must derive from the **deployed** `VELOGB10_MAX_SEQ_LEN`, not the model's 262,144-token native context. Velo needs an additional 16 tokens for speculative decoding, so advertise `VELOGB10_MAX_SEQ_LEN - 16` as the client context window and reserve `VELOGB10_DEFAULT_MAX_TOKENS` for output. For the default stack those values are 65,520 and 8,192 tokens, leaving up to 57,328 prompt tokens. Otherwise a client may postpone history compression until after Velo's real limit. Near the limit, v0.5.0 can clamp `max_tokens` to the nominal remaining KV space without reserving the speculative-decoding margin, reject the request with zero output, and provoke fruitless continuation retries in clients that interpret `context_length_exceeded` as ordinary truncation.
